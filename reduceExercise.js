...
function extractValuSolution(arr, key){
    return arr.reduce(function(accumulator, nextValue){
        accumulator.push(nextValue[key]);
        //can't do this in one line because push returns new length of array, not the array itself.
        //concat from above works, but it's creating a whole new array, which is more expensive than needed
        return accumulator;
    }, []);
}
...
//everything below this works, but I don't think I'm doing them the right way
...
/*
Write a function called addKeyAndValue which accepts an array of objects and returns the array of objects passed to it with each object now including the key and value
...
function addKeyAndValue(arr, key, value){
    return arr.reduce(function(accumulator, nextValue){
        //he combined next 2 lines into acc[idx][key] = value; same idea
        nextValue[key] = value
